...
export const getGeneralAnnouncements = https.onRequest((req, res) => {
  corsHandler(req, res, () => {
    const formatted: GeneralAnnouncement[] = [
      {
        title: 'New App',
        content:
          'This app version is no longer supported. Please use the latest version at https://staugustinecha.ca',
      },
    ]
...
export const saveGeneralAnnouncementG2 = onRequest((req, res) => {
  corsHandler(req, res, async () => {
    try {
      const src: any = req.method === 'POST' ? req.body : req.query
